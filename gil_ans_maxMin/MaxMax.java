...
public class MaxMax {
	...
	public static void main(String[] args) {
		...
		int [] arr= new int [(int)(Math.random()*1000)+10];
		for(int i=0;i<x;i++){
			for (int j = 0; j < arr.length; j++) {
				arr[j] = (int)(Math.random()*(1000)+100);
			}
			an1 = an1 + (double)maxMaxM1(arr)/(double)arr.length;
			
			an2 = an2 + (double)maxMaxM2Recursive(arr)/(double)arr.length;
			an3 = an3 + (double)maxMaxM2Induction(arr)/(double)arr.length;
		}
		System.out.println("an1 :" + an1/x+ "  an2: "+ an2/x + "  an3:  "+ an3/x);
		getAverageofMethod(1,1000,1000);
		getAverageofMethod(2,1000,1000);
		getAverageofMethod(3,1000,1000);
	}
